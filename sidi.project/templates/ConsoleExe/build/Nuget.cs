...
public class NugetPackTools: ITask  
{  
    //When implementing the ITask interface, it is necessary to  
    //implement a BuildEngine property of type  
    //Microsoft.Build.Framework.IBuildEngine. This is done for  
    ...
    {  
        var targetsToPack = this.Targets
            .Where(_ => _.ItemSpec.EndsWith(".exe"))
            .Where(_ => !_.ItemSpec.EndsWith(".Test.dll"));
        ...
        {
            // Util.Dump(Console.Out, target);
            output.Add(new TaskItem(Nuget.PackTool(target.GetMetadata("FileName"), Path.GetDirectoryName(target.GetMetadata("FullPath")), this.OutputDirectory, Version)));
        }
        Outputs = output.ToArray();
